...
// 2. Map current resources to their IDs
// 3. Execute "terraform refresh" to refresh state
// 4. Identity and remove ".tf.json" files that are not in the terraform state
...
func (p *plugin) handleFiles(fns tfFuncs) error {
		...
		if tfStateResName, has := tfStateResourcesAfter[resType]; has {
			...
			for resName, propsFilename := range resNameFilenamePropsMap {
				if _, has = tfStateResName[resName]; has {
					...
					log.Infof("Detected instance %v.%v to prune at file: %v", resType, resName, propsFilename.FileName)
	...
	if err = p.handleOrphans(fns, prunes, tfStateResourcesBefore); err != nil {
		return err
	}
...
// handleOrphans procsses the prune file that are no longer associated with resources
