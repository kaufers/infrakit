...
func GetIBMCloudVMByTag(username, apiKey string, tags []string) (*int, error) {
	...
	vms, err := services.GetAccountService(c.Sess).Mask("id;hostname;tagReferences").GetVirtualGuests()
	if err != nil {
		return nil, err
	}
	// Filter by tags
	...
	if len(vms) == 1 {
		...
		if vms[0].Id == nil {
			return nil, fmt.Errorf("VM missing ID: %v", vms[0])
		}
	...
	return nil, fmt.Errorf("Only a single VM should match tags, but VMs %v match tags: %v", ids, vms[0])
}
